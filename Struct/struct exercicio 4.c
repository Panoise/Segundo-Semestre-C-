...
float maiorNotaUM(BobEsponja x){
    BobEsponja maiornota;
    if (x.prova1 > maiornota.prova1){
        maiornota.prova1 = x.prova1;
    }
    return maiornota.prova1;
}
...
for (i = 1; i <= 5; i++){
    ...
    if (seila[i].media > Maiormediageral || i == 1){
        ...
        printf("O aluno %c foi aprovado!", seila[i].nome);
    }if (seila[i].media < 6){
        printf("O aluno %c foi reprovado!", seila[i].nome);
    }

}
printf("O aluno %c teve a maior média! Sua nota foi de: %d\n", Melhoraluno, Maiormediageral);
printf("O aluno %c teve a pior média! Sua nota foi de: %d\n", pioraluno, Menormediageral);
